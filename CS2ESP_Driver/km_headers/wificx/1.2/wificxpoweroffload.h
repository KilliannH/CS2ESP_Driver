...
typedef enum _WIFI_POWER_OFFLOAD_TYPE
{
    ...
    WifiPowerOffloadTypeWakeOnIncomingActionFrame
...
{
    RtlZeroMemory(Parameters, sizeof(WIFI_POWER_OFFLOAD_80211RSNREKEY_PARAMETERS));
    Parameters->Size = WIFI_STRUCTURE_SIZE(WIFI_POWER_OFFLOAD_80211RSNREKEY_PARAMETERS);
}
...
WIFI_POWER_OFFLOAD_ACTION_FRAME_WAKE_PATTERN_PARAMETERS_INIT(
    _Out_ WIFI_POWER_OFFLOAD_ACTION_FRAME_WAKE_PATTERN_PARAMETERS * Parameters
)
{
    RtlZeroMemory(Parameters, sizeof(WIFI_POWER_OFFLOAD_ACTION_FRAME_WAKE_PATTERN_PARAMETERS));
    Parameters->Size = WIFI_STRUCTURE_SIZE(WIFI_POWER_OFFLOAD_ACTION_FRAME_WAKE_PATTERN_PARAMETERS);
}
...
//
// WIFI Function: WifiPowerOffloadGetActionFrameWakePatternParameters
//
typedef
_IRQL_requires_(PASSIVE_LEVEL)
WDFAPI
void
(NTAPI *PFN_WIFIPOWEROFFLOADGETACTIONFRAMEWAKEPATTERNPARAMETERS)(
    _In_
    PWIFI_DRIVER_GLOBALS DriverGlobals,
    _In_
    WIFIPOWEROFFLOAD WifiPowerOffload,
    ...
    )
{
    ...
    ((PFN_WIFIPOWEROFFLOADGETACTIONFRAMEWAKEPATTERNPARAMETERS) WifiFunctions[WifiPowerOffloadGetActionFrameWakePatternParametersTableIndex])(WifiDriverGlobals, WifiPowerOffload, Parameters);
#else
    if (WIFI_IS_FUNCTION_AVAILABLE(WifiPowerOffloadGetActionFrameWakePatternParameters)) {
        ((PFN_WIFIPOWEROFFLOADGETACTIONFRAMEWAKEPATTERNPARAMETERS) WifiFunctions[WifiPowerOffloadGetActionFrameWakePatternParametersTableIndex])(WifiDriverGlobals, WifiPowerOffload, Parameters);
    }
    else {
        ((PFN_WDFDRIVERERRORREPORTAPIMISSING) WdfFunctions[WdfDriverErrorReportApiMissingTableIndex])(WdfDriverGlobals, WdfGetDriver(), WifiFrameworkExtensionName, WifiPowerOffloadGetActionFrameWakePatternParametersTableIndex, FALSE);
    }
#endif
}
