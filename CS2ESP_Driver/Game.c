...
ULONG64 GetEntityFromHandle(ULONG64 entityList, ULONG handle) {
    ...
    ULONG64 chunkIdx = (handle & 0x7FFF) >> 9;
    ULONG64 slotIdx = handle & 0x1FF;
...
ULONG64 ScanViewMatrix() {
    ...
    ULONG64 foundAddr = KernelPatternScan(g_TargetProcess, g_ClientBase, pat1, msk1, 15);
...
VOID CollectEnemyData(PESP_DATA Data) {
    ...
    ULONG64 localController = 0, localPawn = 0;
    ...
    for (int i = 1; i < 64 && enemyIndex < MAX_ENEMIES; i++) {
        ULONG64 controller = GetEntityFromHandle(entityList, i);
        ...
        ULONG64 pawn = GetEntityFromHandle(entityList, pawnHandle);
        if (!pawn) continue;
